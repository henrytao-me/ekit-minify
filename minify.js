...
(function() {'use strict';
	...
	var DIR = __dirname + '/', LIBDIR = DIR + 'lib/', main = require(LIBDIR + 'main'), img = main.require(LIBDIR + 'img'), crypto = main.crypto, fs = main.fs, path = main.path, Util = main.util, MinFolder = DIR + 'min/';
	...
	function optimize(pFiles, pOptions) {
		var lFiles = Util.isArray(pFiles) ? pFiles : [pFiles], lName = '', lAllCSS = '',
		...
		dataReaded_f = function(pFileData_o) {
			...
			var lFileName = pFileData_o.name, lData = pFileData_o.data, lOptimizeParams;
			...
			var lExt = Util.getExtension(lFileName), lMinFileName = getName(lFileName, lExt);
	...
	function getName(pName, pExt) {
		...
		if(Util.isString(pName)) {
			...
			var lExt = pExt || Util.getExtension(pName), lMinFileName = crypto.createHash('sha1').update(pName).digest('hex') + lExt;
	...
	function fileReaded(pParams) {
		var p, d, lData, lRet = Util.checkObj(pParams, ['error', 'data']) && Util.checkObjTrue(pParams.params, ['name', 'callback']);

		if(lRet) {
			p = pParams, d = p.params;
